            ...
            if (Object.keys(postedSms).length > 0) {
                input.messages.forEach(function(message) {
                    Object.keys(postedSms).forEach(function(key) {
                        ...
                        } else if (Date.now() - posted.timestamp > 15000) {
                            delete postedSms[key];
                        }
                    });
                });
...
var smsChangedListener = function(e) {
    ...
    if (!thread) {
        ...
        pb.get(pb.api + '/v2/permanents/' + parts[0] + '_threads', function(response) {
            if (response) {
...
var updateThread = function() {
    if (thread) {
        ...
        pb.get(pb.api + '/v2/permanents/' + location.hash.substring(1), function(response) {
            if (response) {
                document.getElementById('input').focus();
                var messages = response.thread.reverse();
